...
//    val usedSubsumptionEdges = subsumptionEdges.nonEmpty
    ...
    def replaceFilterEdge(filterEdgeToReplace: FilterEdge, newFilterEdge: FilterEdge): FilterEdge = {
      newFilterEdge ++ filterEdgeToReplace.filter({ case ThenBranchTaken | ElseBranchTaken => true; case _ => false })
    }
    ...
    val nonSubsumptionStateCombos: Set[(EdgeAnnotation2, StateCombo)] = results.flatMap({
      case (newNewState, filterEdge) =>
        ...
        initialGraphFilteredEdge.map((edge) => {
          ...
          val edgeAnnotation = edge._1.copy(_1 = replaceFilterEdge(edge._1._1, filterEdge))

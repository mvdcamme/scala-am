    ...
    trace.foldRight(Set[HybridAddress.A]())((stateInfo, boundAddresses) => stateInfo match {
      case (_, infos) =>
        infos.find[Set[HybridAddress.A]](
          { case AddressesAllocated(_) => true; case AddressesReassigned(_) => true; case _ => false},
          { case AddressesAllocated(addresses) => boundAddresses ++ addresses.toSet; case AddressesReassigned(addresses) => boundAddresses ++ addresses.toSet }).getOrElse(boundAddresses)

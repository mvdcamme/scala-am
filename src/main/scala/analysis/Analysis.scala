...
abstract class BaseAnalysis[
    L, Exp: Expression, Abs: JoinLattice, Addr: Address, Time: Timestamp]
...
case class ProductAnalysis[L1,
                           L2,
                           Exp: Expression,
                           Abs: JoinLattice,
                           Addr: Address,
                           Time: Timestamp](
    analysis1: Analysis[L1, Exp, Abs, Addr, Time],
    analysis2: Analysis[L2, Exp, Abs, Addr, Time])
    extends BaseAnalysis[(L1, L2), Exp, Abs, Addr, Time] {
  def stepEval(e: Exp,
               env: Environment[Addr],
               store: Store[Addr, Abs],
               t: Time,
  ...
  def error(err: SemanticError, current: (L1, L2)) =
    (analysis1.error(err, current._1), analysis2.error(err, current._2))
  def join(x: (L1, L2), y: (L1, L2)) =
    (analysis1.join(x._1, y._1), analysis2.join(x._2, y._2))

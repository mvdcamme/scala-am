...
class SchemeTraceOptimizer[Abs: ConstantableLatticeInfoProvider](
    val sem: SchemeSemanticsTraced[ConcreteConcreteLattice.L,
                                   HybridAddress.A,
                                   HybridTimestamp.T],
    ...
    extends TraceOptimizer[SchemeExp,
                           ConcreteConcreteLattice.L,
                           HybridAddress.A,
                           HybridTimestamp.T] {

  type TraceInstructionInfo = Tracer[SchemeExp,
                                     ConcreteConcreteLattice.L,
                                     HybridAddress.A,
                                     HybridTimestamp.T]#TraceInstructionInfo
  type TraceInstruction = Tracer[SchemeExp,
                                 ConcreteConcreteLattice.L,
                                 HybridAddress.A,
                                 HybridTimestamp.T]#TraceInstruction
  type TraceWithoutStates = Tracer[SchemeExp,
                                   ConcreteConcreteLattice.L,
                                   HybridAddress.A,
                                   HybridTimestamp.T]#TraceWithoutStates
  type Trace = Tracer[SchemeExp,
                      ConcreteConcreteLattice.L,
                      HybridAddress.A,
                      HybridTimestamp.T]#TraceWithInfos

  type SpecTraceFull = TraceFull[SchemeExp,
                                 ConcreteConcreteLattice.L,
                                 HybridAddress.A,
                                 HybridTimestamp.T]
  ...
  val basicOptimizations: List[(Boolean, (SpecTraceFull => SpecTraceFull))] =
    List(
      (GlobalFlags.APPLY_OPTIMIZATION_CONTINUATIONS_LOADING,
       optimizeContinuationLoading(_)),
      (GlobalFlags.APPLY_OPTIMIZATION_ENVIRONMENTS_LOADING,
       optimizeEnvironmentLoading(_)),
  ...
  val detailedOptimizations: List[(Boolean, (SpecTraceFull => SpecTraceFull))] =
    List((GlobalFlags.APPLY_OPTIMIZATION_CONSTANT_FOLDING,
          optimizeConstantFolding(_)),
         (GlobalFlags.APPLY_OPTIMIZATION_TYPE_SPECIALIZED_ARITHMETICS,
          optimizeTypeSpecialization(_)))

  def foldOptimisations(
      traceFull: SpecTraceFull,
      optimisations: List[(Boolean, (SpecTraceFull => SpecTraceFull))])
    : SpecTraceFull = {
  ...
  def optimize(
      trace: SpecTraceFull,
      state: ConcreteTracingProgramState[SchemeExp,
                                         HybridAddress.A,
                                         HybridTimestamp.T]): SpecTraceFull = {
    Logger.log(s"Size of unoptimized trace = ${trace.trace.length}", Logger.V)
    val basicAssertedOptimizedTrace =
      foldOptimisations(trace, basicOptimizations)
    Logger.log(
      s"Size of basic optimized trace = ${basicAssertedOptimizedTrace.trace.length}",
      Logger.V)
    val tier2AssertedOptimizedTrace =
      foldOptimisations(basicAssertedOptimizedTrace, detailedOptimizations)
    Logger.log(
      s"Size of advanced optimized trace = ${tier2AssertedOptimizedTrace.trace.length}",
      Logger.V)
    val tier3AssertedOptimizedTrace =
      if (tracingFlags.SWITCH_ABSTRACT) {
        /* Runtime analyses (+ an initial analysis) performed. */
        val addressesLookedUp = TraceAnalyzer
          .collectAddressesLookedUp[SchemeExp, HybridTimestamp.T](trace.trace)
        val analysisOutput =
          constantsAnalysisLauncher.runStaticAnalysis(state, addressesLookedUp)
        analysisOutput match {
          case ConstantAddresses(_, nonConstants) =>
            val staticallyOptimizedTrace = applyConstantVariablesOptimizations(
              tier2AssertedOptimizedTrace,
              nonConstants.asInstanceOf[Set[HybridAddress.A]])
            Logger.log(
              s"Size of statically optimized trace = ${staticallyOptimizedTrace.trace.length}",
              Logger.V)
    ...
    val finalAssertedOptimizedTrace = removeFunCallBlockActions(
      tier3AssertedOptimizedTrace)
    Logger.log(
      s"Size of final optimized trace = ${finalAssertedOptimizedTrace.trace.length}",
      Logger.V)
   ...
   */
  private case class ActionStateMap(actionState: TraceInstructionInfo,
                                    var isUsed: Boolean)

  private def removeMatchingActions(
      trace: Trace,
      isAPushingAction: TraceInstruction => Boolean,
      isAPoppingAction: TraceInstruction => Boolean,
      isAnInterferingAction: TraceInstruction => Boolean): Trace = {
  ...
  private def createFullTrace(traceFull: SpecTraceFull,
                              optimisedTrace: Trace): SpecTraceFull =
    traceFull.copy(trace = optimisedTrace)

  /********************************************************************************************************************
    *                                         ENVIRONMENT LOADING OPTIMIZATION                                         *
   ********************************************************************************************************************/
  private def optimizeEnvironmentLoading(
      traceFull: SpecTraceFull): SpecTraceFull = {
    def isAnInterferingAction(action: TraceInstruction) = action match {
      case ActionAllocVarsT(_) | ActionEndTrace(_) | ActionExtendEnvT(_) =>
        true
    ...
    val optimizedTrace = removeMatchingActions(
      traceFull.trace,
   ...
   ********************************************************************************************************************/
  private def optimizeContinuationLoading(
      traceFull: SpecTraceFull): SpecTraceFull = {
    def isAnInterferingAction(action: TraceInstruction): Boolean =
      action match {
  ...
  private def findNextAction(
      trace: Trace,
      pred: TraceInstruction => Boolean): Option[(Trace, Trace)] = {
    val (traceBefore, traceAtEndPrimCall) =
      trace.span({ (traceInstructionState) =>
        pred(traceInstructionState._1)
      })
  ...
  private def findNextEndPrimCall(trace: Trace): Option[(Trace, Trace)] = {
    findNextAction(trace, {
      case ActionEndPrimCallT() => false
      case _ => true
    })
  }
  ...
  private def findNextStartFunCall(trace: Trace): Option[(Trace, Trace)] = {
    findNextAction(trace, {
      case ActionStartFunCallT() => false
      case _ => true
    })
  }
  ...
  private def findNextEndOptimizedBlock(trace: Trace): Option[(Trace, Trace)] = {
    findNextAction(trace, {
      case ActionEndOptimizedBlock() => false
      case _ => true
    })
  }

  private def findNextStartOptimizedBlock(
      trace: Trace): Option[(Trace, Trace)] = {
    findNextAction(trace, {
      case ActionStartOptimizedBlock() => false
      case _ => true
    })
  }
  ...
  /* TODO */
  private def changesValueRegister(action: TraceInstruction): Boolean =
    action match {
  ...
  private def checkPrimitive(trace: Trace, n: Integer): Option[Trace] = {
    val onlyUsesConstants = 1
      .to(n - 1)
      .foldLeft(Some(trace): Option[Trace])({
        (previousResult: Option[Trace], x) =>
          previousResult
            .flatMap({ currentTrace =>
              val someTraceAfterPush = findNextPushVal(currentTrace)
              someTraceAfterPush.flatMap({
                traceAfterPush: Trace =>
                  val traceAtValueChangingAction = traceAfterPush.dropWhile({
                    (instructionState) =>
                      !changesValueRegister(instructionState._1)
                  })
                  if (traceAtValueChangingAction.isEmpty) {
                    None
                  } else {
                    if (traceAtValueChangingAction.head._1
                          .isInstanceOf[ActionReachedValueT[
                            SchemeExp,
                            ConcreteValue,
                            HybridAddress.A]]) {
  ...
  private def doOneConstantFold(firstPart: Trace,
                                trace: Trace): Option[Trace] = {
      ...
      case Some((traceBefore, traceAtPrimCall)) =>
        findNextStartFunCall(traceAtPrimCall.tail) match {
          case Some((traceBetweenMarks, traceAtStartCall)) =>
            ...
            val actionStatePrimCall = traceBetweenMarks.find(
              _._1.isInstanceOf[ActionPrimCallT[SchemeExp,
                                                ConcreteValue,
                                                HybridAddress.A]])
                ...
                infos.flatMap[Trace]({
                  ...
                  case PrimitiveAppliedInfo(v, _) =>
                    ...
                    x match {
                      ...
                      case Some(
                          (_, betweenInnerEndPrimCallAndOuterStartMark)) =>
                        val betweenOuterPrimCallAndInnerEndPrimCall =
                          findNextEndPrimCall(traceBetweenMarks).get._1
                      ...
                      case _ =>
                        checkPrimitive(traceBetweenMarks, n).flatMap({
                          (traceAfterOperatorPush) =>
                            //val guard = (ActionGuardSamePrimitive(), None)
                            val replacingConstantAction: TraceInstructionInfo =
                              (ActionReachedValueT[SchemeExp,
                                                   ConcreteValue,
                                                   HybridAddress.A](result),
                               TraceInfos.nil[ConcreteValue, HybridAddress.A])
                            val actionEndOptimizedBlock =
                              (ActionEndOptimizedBlock[SchemeExp,
                                                       ConcreteValue](),
                               TraceInfos.nil[ConcreteValue, HybridAddress.A])
                            val actionStartOptimizedBlock =
                              (ActionStartOptimizedBlock[SchemeExp,
                                                         ConcreteValue](),
                               TraceInfos.nil[ConcreteValue, HybridAddress.A])
                            val replacingTrace = firstPart ++ (traceBefore :+ actionEndOptimizedBlock :+ replacingConstantAction) ++
                                ...
                                optimizedBlocks.foldLeft(List(): Trace)({
                                (acc, current) =>
                                  acc ++ current.filter({ (actionState) =>
                                    !actionState._1
                                      .isInstanceOf[ActionReachedValueT[
                                        SchemeExp,
                                        ConcreteValue,
  ...
  private def optimizeConstantFolding(
      traceFull: SpecTraceFull): SpecTraceFull = {
   ...
   ********************************************************************************************************************/
  private def typeSpecializePrimitive(
      prim: Primitive[HybridAddress.A, ConcreteValue],
      operandsTypes: SimpleTypes.Value)
    : Primitive[HybridAddress.A, ConcreteValue] = prim match {
    case sem.primitives.Plus =>
      operandsTypes match {
        case SimpleTypes.Float => sem.primitives.PlusFloat
        case SimpleTypes.Integer => sem.primitives.PlusInteger
        case _ => prim
      }
    case sem.primitives.Minus =>
      operandsTypes match {
  ...
  private def optimizeTypeSpecialization(
      traceFull: SpecTraceFull): SpecTraceFull = {
      ...
      case (actionState1 @ (_, infos)) :: (actionState2 @ (
        ...
        infos.find[Trace]({
          ...
          case PrimitiveAppliedInfo(_, vStack) =>
            ...
            val latticeInfoProvider =
              implicitly[LatticeInfoProvider[ConcreteConcreteLattice.L]]
            val operandsTypes = latticeInfoProvider.simpleTypes(operands)
            val somePrimitive = sabs
              .getPrimitives[HybridAddress.A, ConcreteValue](supposedOperator)
              .headOption
            somePrimitive match {
              case None =>
                throw new Exception(
                  s"Operation being type-specialized is not a primitive: $supposedOperator")
              case Some(primitive) =>
                val specializedPrim =
                  typeSpecializePrimitive(primitive, operandsTypes)
                val restartPoint =
                  RestartSpecializedPrimitive(primitive, n, fExp, argsExps)
                val specializedPrimGuard =
                  ActionGuardSpecializedPrimitive[SchemeExp,
                                                  ConcreteValue,
                                                  HybridAddress.A](
                    operandsTypes,
                    nrOfArgs,
                ...
                val specializedPrimCallAction =
                  ActionSpecializePrimitive[SchemeExp,
                                            ConcreteValue,
                                            HybridAddress.A](operandsTypes,
                                                             specializedPrim,
                                                             n,
                                                             fExp,
                                                             argsExps)
                loop(
                  rest,
                  (specializedPrimCallAction, actionState2._2) :: (specializedPrimGuard,
                                                                   infos) :: actionState1 :: acc)
            }
        }) match {
  ...
  def optimizeVariableFolding(traceFull: SpecTraceFull): SpecTraceFull = {
    ...
    val boundAddresses = TraceAnalyzer
        .collectTraceBoundAddresses[SchemeExp, HybridTimestamp.T](
          traceFull.trace) ++ initialBoundAddresses
    ...
    def replaceVariableLookups(
      ...
      someNewTraceInfo match {
        case None =>
          throw new Exception(
            s"Error: ActionLookUpT without an associated VariableLookedUp info: $action")
    ...
    TraceFull(traceFull.info,
              traceFull.assertions ++ putRegisterActions,
              optimisedTrace)
  }
  ...
  /**
    ...
    */
  def possiblyOptimizeVariableFolding(
      traceFull: SpecTraceFull): SpecTraceFull = {
  ...
  def optimizeMergeActions(traceFull: SpecTraceFull): SpecTraceFull = {
    ...
    def loop(trace: Trace, acc: Trace): Trace = trace match {
      case Nil => acc.reverse
      case (ActionLookupVariableT(varName, read, write), i1) :: (
          ActionPushValT(),
          i2) :: rest =>
        loop(rest,
             (ActionLookupVariablePushT[SchemeExp,
                                        ConcreteValue,
      ...
      case (ActionRestoreEnvT(), i1) :: (ActionSaveEnvT(), i2) :: rest =>
        loop(
          rest,
          (ActionRestoreSaveEnvT[SchemeExp, ConcreteValue, HybridAddress.A](),
           i1.join(i2)) :: acc)
...
//  }
  ...
  /**
    ...
    *         of the trace.
    */
  private def findAllBoundAddresses(
      traceFull: SpecTraceFull,
      traceExteriorBoundAddresses: Set[HybridAddress.A])
    : Set[HybridAddress.A] = {
    /* The addresses that are bound at the start of the trace. */
    val initialBoundAddresses = traceFull.info.boundVariables.map({
     ...
     * within the trace. */
    val traceBoundAddresses =
      TraceAnalyzer.collectTraceBoundAddresses[SchemeExp, HybridTimestamp.T](
        traceFull.trace)
  ...
  private def addressBound(address: HybridAddress.A,
                           boundAddresses: Set[HybridAddress.A]): Boolean = {
    val abstractAddress =
      new DefaultHybridAddressConverter[SchemeExp]().convertAddress(address)
    /* Check both the abstracted address and the concrete address */
    boundAddresses.contains(abstractAddress) || boundAddresses.contains(
      address)
  }

  private def replaceVariableLookups(
      trace: Trace,
      boundAddresses: Set[HybridAddress.A]): Trace = {

    def replaceVariableLookup(
        address: HybridAddress.A,
        value: ConcreteValue,
      ...
      if (addressBound(address, boundAddresses)) {
        ...
        Logger.log(
          s"Replaced variable lookup for address $address with new actionInfo $newActionInfo",
          Logger.V)
        newActionInfo
      }
    }

    def replaceActionInfo(
        actionInfo: TraceInstructionInfo): TraceInstructionInfo =
      actionInfo match {
        case (ActionLookupVariableT(_, _, _), infos) =>
          infos
            .find[TraceInstructionInfo]({
              ...
              case VariableLookedUp(_, address, value) =>
                val newActionInfo =
                  (ActionReachedValueT[SchemeExp,
                                       ConcreteValue,
                                       HybridAddress.A](value),
                   infos
                     .filter[VariableLookedUp[ConcreteValue, HybridAddress.A]])
                replaceVariableLookup(address,
                                      value,
                                      actionInfo,
                                      newActionInfo)
            })
            .getOrElse(actionInfo)
        case (ActionLookupVariablePushT(_, _, _), infos) =>
          infos
            .find[TraceInstructionInfo]({
              ...
              case VariableLookedUp(_, address, value) =>
                val newActionInfo =
                  (ActionReachedValuePushT[SchemeExp,
                                           ConcreteValue,
                                           HybridAddress.A](value),
                   infos
                     .filter[VariableLookedUp[ConcreteValue, HybridAddress.A]])
  ...
  private def replaceVariablesWithConstants(
      traceFull: SpecTraceFull,
      allBoundAddresses: Set[HybridAddress.A]): SpecTraceFull = {
    Logger.log(
      s"Initiating new variable folding optimization; allBoundAddresses = $allBoundAddresses",
      Logger.D)
    val optimizedTrace =
      replaceVariableLookups(traceFull.trace, allBoundAddresses)
    createFullTrace(traceFull, optimizedTrace)
  }
   ...
   *********************************************************************************************************************/
  private def removeRedundantClosureGuards(
      traceFull: SpecTraceFull,
      allBoundAddresses: Set[HybridAddress.A]): SpecTraceFull = {
    Logger.log(
      s"Initiating closure guards removal optimization; allBoundAddresses = $allBoundAddresses",
      Logger.D)
        ...
        case ActionLookupVariableT(variable, _, _) =>
          infos
            .find[Boolean]({
              ...
              case VariableLookedUp(_, address, _) =>
                addressBound(address, allBoundAddresses)
            })
            .getOrElse(false)
    ...
    def removeMatchingClosureGuard(traceSoFar: Trace,
                                   remainingTrace: Trace): (Trace, Trace) =
      remainingTrace match {
        case Nil =>
          (traceSoFar, Nil)
    ...
    def findNextFunctionEndBlock(traceSoFar: Trace,
                                 remainingTrace: Trace): (Trace, Trace) =
      remainingTrace match {
        case Nil =>
          (traceSoFar, Nil)
    ...
    /**
      ...
      */
    def inStartsFunCallBlock(
        traceSoFar: Trace,
        remainingTrace: Trace,
        changesValueReg: Option[TraceInstructionInfo]): (Trace, Trace) =
      remainingTrace match {
        case Nil =>
          (traceSoFar, Nil)
    ...
    def loop(traceSoFar: Trace, remainingTrace: Trace): Trace =
      remainingTrace match {
        case Nil =>
          traceSoFar
        case (tuple @ (action, infos)) :: rest =>
          if (action.startsFunCallBlock) {
            val (newTraceSoFar, newRest): (Trace, Trace) =
              inStartsFunCallBlock(traceSoFar :+ tuple, rest, None)

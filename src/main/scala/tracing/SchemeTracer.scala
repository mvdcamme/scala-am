...
class SchemeTracer[Abs: JoinLattice, Addr: Address, Time: Timestamp](
  ...
  def startTracingLoop(
      ...
      startState: ConcreteTracingProgramState[SchemeExp, Addr, Time])
  ...
  def startTracingGuard(
    ...
    case SchemeTracerContext(labelCounters, traceNodes, _) =>
      ...
      val parentTraceInitialBoundVariables =
        parentTraceNode.trace.info.boundVariables
      val allInitialBoundVariables = boundVariables ++ parentTraceInitialBoundVariables
      Logger.log(
        s"Including initial bound variables of parent loop trace $parentTraceInitialBoundVariables for guard trace",
        Logger.V)
      val label = GuardLabel(loopID, guardID)
      val traceInfo =
        TraceInfo(allInitialBoundVariables, startState, Some(parentTraceLabel))
      val traceNode = TraceNode[Trace](label, Nil, traceInfo)
      SchemeTracerContext(labelCounters, traceNodes, Some(traceNode))
  }
  ...
  def stopTracing(tc: TracerContext,
                  isLooping: Boolean,
                  traceEndedInstruction: Option[TraceInstruction],
                  state: ConcreteTracingProgramState[SchemeExp, Addr, Time])
    : SchemeTracerContext = {
    var finishedTc: SchemeTracerContext = tc
    if (!isLooping) {
      finishedTc = appendTrace(
        tc,
        List((traceEndedInstruction.get, TraceInfos.nil[Abs, Addr])))
    }
  ...
  private def searchTraceWithLabelMatching(
      tc: TracerContext,
      labelPred: Label[SchemeExp] => Boolean)
    : Option[TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]]] =
    tc.traceNodes.find({ traceNode =>
      labelPred(traceNode.label)
    })

  private def searchLoopTrace(tc: TracerContext, loopID: List[SchemeExp])
    : Option[TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]]] =
    searchTraceWithLabelMatching(tc, NormalLabel(loopID) == _)

  private def searchGuardTrace(tc: TracerContext, guardID: Integer)
    : Option[TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]]] =
    searchTraceWithLabelMatching(tc, {
      case NormalLabel(_) => false
      case GuardLabel(_, guardID2) => guardID == guardID2
    })

  private def getTrace(tc: TracerContext, label: Label[SchemeExp])
    : TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]] =
    searchTraceWithLabelMatching(tc, _ == label) match {
      case Some(traceNode) => traceNode
      case None =>
        throw new Exception(
          s"Retrieving non-existing trace (should not happen): $label")
    }

  def getLoopTrace(tc: TracerContext, loopID: List[SchemeExp])
    : TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]] =
    searchLoopTrace(tc, loopID) match {
      case Some(traceNode) => traceNode
      case None =>
        throw new Exception(
          s"Retrieving non-existing loop-trace (should not happen): ${NormalLabel(loopID)}")
    }

  def getGuardTrace(
      tc: TracerContext,
      guardID: Integer): TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]] =
    searchGuardTrace(tc, guardID) match {
      case Some(traceNode) => traceNode
      case None =>
        throw new Exception(
          s"Retrieving non-existing guard-trace (should not happen): guardID = $guardID")
    }

  def loopTraceExists(tc: TracerContext, loopID: List[SchemeExp]): Boolean =
    searchLoopTrace(tc, loopID) match {
  ...
  def appendTrace(tc: TracerContext, newPart: Trace): SchemeTracerContext = {
    val newCurTraceNode = tc.curTraceNode.get
      .copy(trace = newPart.reverse ++ tc.curTraceNode.get.trace)
  ...
  def isTracingLoop(tc: TracerContext, loopID: List[SchemeExp]): Boolean =
  ...
  def stepTrace(
      traceNode: TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]],
      tc: TracerContext): (TraceInstruction,
                           TraceNode[TraceFull[SchemeExp, Abs, Addr, Time]],
                           Boolean) = {
    ...
    val updatedTraceNode = currentTraceNode.copy(
      trace =
        currentTraceNode.trace.copy(trace = currentTraceNode.trace.trace.tail))
  ...
  private def addTrace(
      tc: TracerContext,
      state: ConcreteTracingProgramState[SchemeExp, Addr, Time])
    : SchemeTracerContext = tc match {
    case SchemeTracerContext(labelCounters, traceNodes, Some(curTraceNode)) =>
      val curInfo = curTraceNode.info
      val newTraceInfo = TraceInfo[SchemeExp, Addr, Time](
        curInfo.boundVariables,
        curInfo.startState,
        curInfo.parentTraceLabel)
      val traceFull = TraceFull[SchemeExp, Abs, Addr, Time](
        newTraceInfo,
        List(),
        curTraceNode.trace.reverse)
      val optimizedTraceFull: TraceFull[SchemeExp, Abs, Addr, Time] =
        someTraceOptimizer match {
  ...
  def incLabelCounter(tc: TracerContext,
                      loopID: List[SchemeExp]): SchemeTracerContext = {
    val oldCounter = getLabelCounter(tc, loopID)
    val newLabelCounters: Map[List[SchemeExp], Integer] =
      tc.labelCounters.updated(loopID, oldCounter + 1)

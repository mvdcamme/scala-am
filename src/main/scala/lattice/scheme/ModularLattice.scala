...
Sym : SymbolLattice
    ...
    def unaryOp(op: UnaryOperator)(x: Value): MayFail[Value] = if (x == Bot) {
      ...
      op match {
        ...
        case StringToNumber => x match {
          case Str(s) => Int(StringLattice[S].toNumber(s))
  ...
  val isConvertableSchemeLattice: IsConvertableLattice[L] = new IsConvertableLattice[L] {
    ...
    def injectVector[Addr: Address](size: scala.Int, elements: Map[scala.Int, Addr], init: Addr): L = {
      val newMap = elements.mapKeys[I]((i) => IntLattice[I].inject(i))
...
//  import PointsToInteger._
  ...
  val constants: Set[Int] = Set[Int](-1, 0, 1, 2, 3, 6, 7, 10, 90, 180, 270)
  val intervalInt = new IntervalInteger(constants)
  val lattice = new MakeSchemeLattice[S, Concrete.B, intervalInt.I, F, C, Concrete.Sym](counting)
  type L = lattice.L

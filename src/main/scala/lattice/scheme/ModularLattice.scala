...
class MakeSchemeLattice[
  ...
  case class Closure[Exp: Expression, Addr: Address](
    lambda: Exp,
  ...
  val isSchemeLatticeValue: IsSchemeLattice[Value] = new IsSchemeLattice[Value] {
    ...
    def subsumes(x: Value, y: Value): Boolean = if (x == y) {
      true
    } else {
      (x, y) match {
        case (_, Bot) => true
    ...
    def unaryOp(op: UnaryOperator)(x: Value): MayFail[Value] = if (x == Bot) {
      Bot
    } else {
      op match {
        case IsNull => x match {
        ...
        case IntegerToChar => x match {
          case Int(i) => Char(IntLattice[I].toChar(i))
          case _ => OperatorNotApplicable("integer->char", List(x.toString))
        }
        ...
        case StringToNumber => x match {
          case Str(s) =>
            val mf: MayFail[I] = StringLattice[Str].stringToNumber(s)
            val set = mf.collect[L]( (i: I) => Set(Int(i)), (err: SemanticError) => Set(Bool(BoolLattice[B].inject(false))))
            set.fold(bottom)(join)
          case _ => OperatorNotApplicable("string->number", List(x.toString))
        }
    ...
    def binaryOp(op: BinaryOperator)(x: Value, y: Value): MayFail[Value] = if (x == Bot || y == Bot) {
      Bot
    } else {
      op match {
        case Plus => (x, y) match {
    ...
    private def wrap(x: => Value): L = try {
      Element(x)
    } catch {
    ...
    val isSchemeLattice = new IsSchemeLattice[L] {
      ...
      def unaryOp(op: UnaryOperator)
        (x: L): MayFail[L] = foldMapL(x, x => isSchemeLatticeValue.unaryOp(op)(x).map(x => wrap(x)))

      def binaryOp(op: BinaryOperator)
        (
          x: L,
          y: L
        ): MayFail[L] = foldMapL(x, x => foldMapL(y, y => isSchemeLatticeValue.binaryOp(op)(x, y).map(x => wrap(x))))
     ...
     * best, i.e., Value(int: I, bool: B, ..., prims: Set[Primitive]) */
      ...
      def vectorRef[Addr: Address](vector: L, index: L): MayFail[Set[Addr]] = foldMapL(vector, vector => foldMapL(index,
        index =>
      ...
      def vectorSet[Addr: Address](
        ...
        index: L,
        addr: Addr
      ): MayFail[(L, Set[Addr])] = foldMapL(vector, vector => foldMapL(index, index =>

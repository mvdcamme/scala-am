...
package object PrimitivesDefinitions {

//  type FunctionCallRequestMF[Exp, Abs, Addr] = MayFail[(FunctionCallRequest[Exp, Abs, Addr], Set[Effect[Addr]])]
...
//  type SimpleReturnSet[Abs, Addr] = (Set[Abs], Store[Addr, Abs], Set[Effect[Addr]])
  ...
  case class StartFunctionCallRequest[Exp: Expression, Abs: JoinLattice, Addr: Address](
    fexp: Exp,
    f: Either[Primitive[Addr, Abs], (Exp, Environment[Addr])],
    args: List[Abs],
...
trait PrimitiveAppState
case object DummyPrimitiveAppState extends PrimitiveAppState
...
/**
  ...
  */
abstract class NonHigherOrderPrimitive[Addr: Address, Abs: JoinLattice] extends Primitive[Addr, Abs] {

  /**
    * Converts the MayFail value returned by non-higher-order primitives to a PrimReturn.
...
abstract class Primitives[Addr: Address, Abs: JoinLattice] {
  ...
  def traced(prim: Primitive[Addr, Abs]): Primitive[Addr, Abs] =
    new Primitive[Addr, Abs] {
      val name = prim.name
      def call[Exp: Expression, Time: Timestamp](fexp: Exp,
        ...
        print("($name $argsstr) -> ")
        res.map({
          case (primReturns, store, effects) =>
            (primReturns.map({
              case ReturnResult(MayFailError(errs)) =>
                "ERROR: " + errs.mkString(" | ERROR: ")
              case ReturnResult(MayFailSuccess((v, store, effs))) =>
                v.toString
  ...
  def convertPrimitive[OtherAbs: IsConvertableLattice](
      prims: SchemePrimitives[Addr, OtherAbs],
      x: Primitive[Addr, Abs]): Primitive[Addr, OtherAbs]

  lazy val bindings = ("bottom", addr.primitive("__bottom__"), lat.bottom) :: all
      .map({ prim =>
      (prim.name, addr.primitive(prim.name), toVal(prim))
    })
}

/** This is where we define Scheme primitives */
class SchemePrimitives[Addr: Address, Abs: IsSchemeLattice]
    extends Primitives[Addr, Abs] {
  import SchemeOps._
  ...
  abstract class NoStoreOperation(val name: String,
      ...
      extends NonHigherOrderPrimitive[Addr, Abs] {
    ...
    def call[Exp: Expression](arg1: (Exp, Abs),
                              arg2: (Exp, Abs)): MayFail[Abs] =
    ...
    def call[Exp: Expression, Time: Timestamp](
      ...
      }).map[SimpleReturn[Abs, Addr]](v => (v, store, Set()))
    }
  }
  ...
  abstract class StoreOperation(val name: String,
    ...
    def call(args: List[Abs], store: Store[Addr, Abs])
      : MayFail[SimpleReturn[Abs, Addr]] =
      MayFailError[SimpleReturn[Abs, Addr]](List(ArityError(name, nargs.getOrElse(-1), args.length)))
    def call(arg: Abs, store: Store[Addr, Abs])
      : MayFail[SimpleReturn[Abs, Addr]] =
      call(List(arg), store)
    def call(arg1: Abs, arg2: Abs, store: Store[Addr, Abs])
      : MayFail[SimpleReturn[Abs, Addr]] =
  ...
  private def applyGenericPlusOperation(
      args: List[Abs],
      reduce: (Abs, Abs) => MayFail[Abs]): MayFail[Abs] = args match {
    case Nil => MayFailSuccess(abs.inject(0))
    case x :: rest =>
      applyGenericPlusOperation(rest, reduce).bind(y => reduce(x, y))
  }
  ...
  object Oddp extends NoStoreOperation("odd?", Some(1)) {
    override def call(x: Abs) =
      modulo(x, abs.inject(2)).bind(mod => numEq(abs.inject(1), mod))
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.Oddp
  }
  ...
  object Evenp extends NoStoreOperation("even?", Some(1)) {
    override def call(x: Abs) =
      modulo(x, abs.inject(2)).bind(mod => numEq(abs.inject(0), mod))
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.Evenp
  }
  object Max extends NoStoreOperation("max") {
    ...
    private def call(args: List[Abs], max: Abs): MayFail[Abs] = args match {
      ...
      case x :: rest =>
        lt(max, x).bind(test => {
          val t = if (abs.isTrue(test)) { call(rest, x) } else {
            MayFailSuccess(abs.bottom)
          }
          val f = if (abs.isFalse(test)) { call(rest, max) } else {
            MayFailSuccess(abs.bottom)
          }
    ...
    private def call(args: List[Abs], min: Abs): MayFail[Abs] = args match {
      ...
      case x :: rest =>
        lt(x, min).bind(test => {
          ...
          val f = if (abs.isFalse(test)) { call(rest, min) } else {
            MayFailSuccess(abs.bottom)
          }
          MayFail.monoid[Abs].append(t, f)
        })
  ...
  object Abs extends NoStoreOperation("abs", Some(1)) {
    override def call(x: Abs) =
      lt(x, abs.inject(0)).bind(ltres => {
        val t = if (abs.isTrue(ltres)) { minus(abs.inject(0), x) } else {
          MayFailSuccess(abs.bottom)
        }
        val f = if (abs.isFalse(ltres)) { MayFailSuccess(x) } else {
          MayFailSuccess(abs.bottom)
        }
        MayFail.monoid[Abs].append(t, f)
      })
    ...
    private def gcd(a: Abs, b: Abs, visited: Set[(Abs, Abs)]): MayFail[Abs] = {
      if (visited.contains((a, b))) {
        ...
        numEq(b, abs.inject(0)).bind(cond => {
          val t = if (abs.isTrue(cond)) { MayFailSuccess(a) } else {
            MayFailSuccess(abs.bottom)
          }
  ...
  object Realp extends NoStoreOperation("real?", Some(1)) {
    override def call(x: Abs) =
      isInteger(x).bind(isint =>
        isFloat(x).map(isfloat => abs.or(isint, isfloat)))
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.Realp
  }
  ...
  object Display extends NoStoreOperation("display", Some(1)) {
    override def call(x: Abs) = {
      val str = x.toString
      print(if (str.startsWith("\"")) { str.substring(1, str.size - 1) } else {
        str
      })
  ...
  object Error extends NoStoreOperation("error", Some(1)) {
    override def call[Exp: Expression](fexp: Exp, x: (Exp, Abs)) =
      MayFailError(
        List(UserError(x._2.toString, implicitly[Expression[Exp]].pos(fexp))))
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.Error
  }
  ...
  def convertPrimitive[OtherAbs: IsConvertableLattice](
      otherPrims: SchemePrimitives[Addr, OtherAbs],
      x: Primitive[Addr, Abs]): Primitive[Addr, OtherAbs] =
    x.convert(otherPrims)
  ...
  object Cons extends NonHigherOrderPrimitive[Addr, Abs] {
    val name = "cons"
    def call[Exp: Expression, Time: Timestamp](fexp: Exp,
                                                     args: List[(Exp, Abs)],
                                                     store: Store[Addr, Abs],
      ...
      case (carexp, car) :: (cdrexp, cdr) :: Nil => {
        ...
        MayFailSuccess[SimpleReturn[Abs, Addr]]((abs.cons(cara, cdra),
                        store.extend(cara, car).extend(cdra, cdr),
                        Set()))
      }
      case l => MayFailError[SimpleReturn[Abs, Addr]](List(ArityError(name, 2, l.size)))
    }
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.Cons
  ...
  private def car(
      v: Abs,
      store: Store[Addr, Abs]): MayFail[(Abs, Set[Effect[Addr]])] = {
    val addrs = abs.car(v)
    if (addrs.isEmpty) {
  ...
  private def cdr(
    ...
    if (addrs.isEmpty) {
      ...
      addrs.foldLeft(mfmon.zero)(
        (acc, a) =>
          MayFail
            .monoid[(Abs, Set[Effect[Addr]])]
            .append(acc, store.lookup(a) match {
  ...
  object SetCar extends StoreOperation("set-car!", Some(2)) {
    override def call(cell: Abs, value: Abs, store: Store[Addr, Abs]) = {
      ...
      if (addrs.isEmpty) {
        MayFailError[SimpleReturn[Abs, Addr]](List(CannotAccessCar(cell.toString)))
      } else {
        val (store2, effects) =
          addrs.foldLeft((store, Set[Effect[Addr]]()))((acc, a) =>
  ...
  object SetCdr extends StoreOperation("set-cdr!", Some(2)) {
    override def call(cell: Abs, value: Abs, store: Store[Addr, Abs]) = {
      ...
      if (addrs.isEmpty) {
        MayFailError[SimpleReturn[Abs, Addr]](List(CannotAccessCdr(cell.toString)))
      } else {
        val (store2, effects) =
          addrs.foldLeft((store, Set[Effect[Addr]]()))((acc, a) =>
  ...
  object Length extends StoreOperation("length", Some(1)) {
    override def call(l: Abs, store: Store[Addr, Abs]) = {
      def length(l: Abs,
                 visited: Set[Abs]): MayFail[(Abs, Set[Effect[Addr]])] = {
        if (visited.contains(l)) {
          ...
          isCons(l).bind(cond => {
            val t = if (abs.isTrue(cond)) {
              cdr(l, store).bind({
                case (cdrl, effects1) =>
                  length(cdrl, visited + l).bind({
                    case (lengthcdrl, effects2) =>
            ...
            val f = if (abs.isFalse(cond)) {
              isNull(l).bind(fcond => {
                val ft = if (abs.isTrue(fcond)) {
                  MayFailSuccess((abs.inject(0), Set[Effect[Addr]]()))
                } else { mfmon.zero }
                val ff = if (abs.isFalse(fcond)) {
                  err(TypeError("length", "first operand", "list", "non-list"))
                } else { mfmon.zero }
                mfmon.append(ft, ff)
              })
        ...
        if (visited.contains(l)) {
          ...
          isNull(l).bind(nulltest => {
            val t = if (abs.isTrue(nulltest)) {
              MayFailSuccess((nulltest, Set[Effect[Addr]]()))
            } else { MayFailSuccess((abs.bottom, Set[Effect[Addr]]())) }
            val f = if (abs.isFalse(nulltest)) {
              isCons(l).bind(constest => {
                val ft = if (abs.isTrue(constest)) {
                  cdr(l, store).bind({
                    case (cdrl, effects1) =>
                      listp(cdrl, visited + l).map({
                        case (listpl, effects2) =>
  ...
  object VectorSet extends NonHigherOrderPrimitive[Addr, Abs] {
    val name = "vector-set!"
    def call[Exp: Expression, Time: Timestamp](fexp: Exp,
                                                     args: List[(Exp, Abs)],
                                                     store: Store[Addr, Abs],
      ...
      case (_, vector) :: (_, index) :: (exp, value) :: Nil => {
        ...
        if (addrs.isEmpty) {
          MayFailError[SimpleReturn[Abs, Addr]](List(CannotAccessVector(vector.toString)))
        } else {
          val init: MayFail[SimpleReturn[Abs, Addr]] =
            MayFailSuccess((abs.bottom, store, Set[Effect[Addr]]()))
          addrs.foldLeft(init)((acc, va) => {
            store.lookup(va) match {
              case Some(oldvec) => {
                acc.bind({
                  case (oldval, store, effects) =>
                    val targetaddr = addr.cell(exp, t)
                    abs
                      .vectorSet(oldvec, index, targetaddr)
                      .map({
                        case (vec, addrs) =>
                          val store2 = addrs.foldLeft(store.update(va, vec))(
                            (st, a) => st.updateOrExtend(a, value))
      ...
      case l => MayFailError[SimpleReturn[Abs, Addr]](List((ArityError(name, 3, l.size))))
    }
    def convert[Addr: Address, Abs: IsConvertableLattice](
        prims: SchemePrimitives[Addr, Abs]): Primitive[Addr, Abs] =
      prims.VectorSet
  }
  object Vector extends NonHigherOrderPrimitive[Addr, Abs] {
    val name = "vector"
    def call[Exp: Expression, Time: Timestamp](fexp: Exp,
                                                     args: List[(Exp, Abs)],
                                                     store: Store[Addr, Abs],
      ...
      convertMayFail(abs
        .vector(a, abs.inject(args.size), botaddr)
        .bind({
          case (va, emptyVector) =>
            /* No tracked effects because we only perform atomic updates at allocation time */
            val init: MayFail[(Abs, Store[Addr, Abs])] =
              MayFailSuccess((emptyVector, store))
            args.zipWithIndex
              .foldLeft(init)((acc, arg) =>
                acc.bind({
                  case (vec, store) =>
                    arg match {
                      case ((exp, value), index) =>
                        ...
                        abs
                          .vectorSet(vec, abs.inject(index), valaddr)
                          .map({
                            case (vec, addrs) =>
                              (vec,
                               addrs.foldLeft(store)((st, a) =>
                                 st.updateOrExtend(a, value)))
                          })
                    }
                }))
              .map({
                case (vector, store) =>
                  (va, store.extend(a, vector), Set[Effect[Addr]]())
              })
        }))
    }
    def convert[Addr: Address, Abs: IsConvertableLattice](
        ...
        addrs.foldLeft(mfmon.zero)(
          (acc, va) =>
            MayFail
              .monoid[(Abs, Set[Effect[Addr]])]
              .append(
                acc, /* TODO: that's basically a foldMap over a monoid... */
                store.lookup(va) match {
                  case Some(v) =>
                    vectorLength(v).map(vl =>
                      (vl, Set(EffectReadVariable(va))))
              ...
              abs
                ...
                .bind(vs =>
                  vs.foldLeft(acc)((acc, a) =>
                    mfmon.append(acc, store.lookup(a) match {
                      case Some(value) =>
                        MayFailSuccess((value, Set(EffectReadVector(a))))
                      case None => err(UnboundAddress(a.toString))
                    })))
            case None => acc.addError(UnboundAddress(va.toString))
        })
  ...
  object Equal extends StoreOperation("equal?", Some(2)) {
    override def call(a: Abs, b: Abs, store: Store[Addr, Abs]) = {
      def equalVec(a: Abs,
                   ...
                   i: Abs,
                   n: Abs,
                   visitedEqual: Set[(Abs, Abs)],
                   visited: Set[(Abs, Abs, Abs, Abs)])
        : MayFail[(Abs, Set[Effect[Addr]])] = {
        if (visited.contains((a, b, i, n)) || a == abs.bottom || b == abs.bottom || i == abs.bottom || n == abs.bottom) {
          MayFailSuccess((abs.bottom, Set[Effect[Addr]]()))
        } else {
          numEq(i, n).bind(numtest => {
            val t = if (abs.isTrue(numtest)) {
              MayFailSuccess((abs.inject(true), Set[Effect[Addr]]()))
            } else { MayFailSuccess((abs.bottom, Set[Effect[Addr]]())) }
                ...
                .bind({
                  case (vai, effects1) =>
                    VectorRef
                      ...
                      .bind({
                        case (vbi, effects2) =>
                          equalp(vai, vbi, visitedEqual).bind({
                            case (itemtest, effects3) => {
                              val tt = if (abs.isTrue(itemtest)) {
                                plus(i, abs.inject(1)).bind(
                                  iplus1 =>
                                    equalVec(a,
                                             b,
                                             iplus1,
                              ...
                              val tf = if (abs.isFalse(itemtest)) {
                                MayFailSuccess(
                                  (abs.inject(false),
                                   effects1 ++ effects2 ++ effects3))
                              } else {
      ...
      def equalp(
          a: Abs,
          b: Abs,
          visited: Set[(Abs, Abs)]): MayFail[(Abs, Set[Effect[Addr]])] = {
        if (visited.contains((a, b)) || a == abs.bottom || b == abs.bottom) {
          MayFailSuccess((abs.bottom, Set[Effect[Addr]]()))
        } else {
          val visited2 = visited + ((a, b))
          abs
            .binaryOp(SchemeOps.Eq)(a, b)
            .bind(eqtest => {
              val t = if (abs.isTrue(eqtest)) {
                MayFailSuccess((eqtest, Set[Effect[Addr]]()))
              } else { MayFailSuccess((abs.bottom, Set[Effect[Addr]]())) }
              val f = if (abs.isFalse(eqtest)) {
                isNull(a).bind(anull =>
                  isNull(b).bind(bnull => {
                    val nulltest = abs.and(anull, bnull)
                    val ft = if (abs.isTrue(nulltest)) {
                      MayFailSuccess((eqtest, Set[Effect[Addr]]()))
                    } else {
                      MayFailSuccess((abs.bottom, Set[Effect[Addr]]()))
                    }
                          ...
                          val fft = if (abs.isTrue(constest)) {
                            car(a, store).bind({
                              case (acar, effects1) =>
                                car(b, store).bind({
                                  case (bcar, effects2) =>
                                    equalp(acar, bcar, visited2).bind({
                                      case (cartest, effects3) => {
                                        val fftt = if (abs.isTrue(cartest)) {
                                          cdr(a, store).bind({
                                            case (acdr, effects4) =>
                                              cdr(b, store).bind({
                                                case (bcdr, effects5) =>
                                                  equalp(acdr, bcdr, visited2)
                                                    .map({
                                                      case (eqtest,
                                                            effects6) =>
                                                        (eqtest,
                                        ...
                                        val fftf = if (abs.isFalse(cartest)) {
                                          MayFailSuccess(
                                            (abs.inject(true)),
                                            effects1 ++ effects2 ++ effects3)
                                        } else {
                              ...
                              isVector(b).bind(bvec => {
                                ...
                                val ffft = if (abs.isTrue(vectest)) {
                                  VectorLength
                                    .length(a, store)
                                    .bind({
                                      case (alength, effects1) =>
                                        VectorLength
                                          .length(b, store)
                                          .bind({
                                            case (blength, effects2) =>
                                              numEq(alength, blength).bind(
                                                lengthtest => {
                                                  val ffftt =
                                                    if (abs.isTrue(lengthtest)) {
                                                      equalVec(a,
                                                               b,
                                                               abs.inject(0),
                                                               alength,
                                ...
                                val ffff = if (abs.isFalse(vectest)) {
                                  MayFailSuccess(
  ...
  object Apply extends Primitive[Addr, Abs] {
    ...
    val listMfmon = MayFail.monoid[(List[Abs], Set[Effect[Addr]])]

    private def collectArguments(lst: Abs, store: Store[Addr, Abs]): MayFail[(List[Abs], Set[Effect[Addr]])] = {
      def loop(l: Abs, visited: Set[Abs]): MayFail[(List[Abs], Set[Effect[Addr]])] = {
        if (visited.contains(l)) {
          MayFailSuccess((Nil, Set[Effect[Addr]]()))
        } else {
          isCons(l).bind(cond => {
            val t = if (abs.isTrue(cond)) {
              car(l, store).bind({
                  ...
                  cdr(l, store).bind({
                    case (cdrl, effects2) =>
                      loop(cdrl, visited + l).bind({
                        case (lst, effects3) =>
                          MayFailSuccess((carl :: lst, effects1 ++ effects2 ++ effects3))
                      })
                  })
              })
            } else { listMfmon.zero }
            val f: MayFail[(List[Abs], Set[Effect[Addr]])] = if (abs.isFalse(cond)) {
              isNull(l).bind(fcond => {
                val ft = if (abs.isTrue(fcond)) {
                  val nil: List[Abs] = Nil
                  MayFailSuccess((nil, Set[Effect[Addr]]()))
                } else { listMfmon.zero }
                val ff: MayFail[(List[Abs], Set[Effect[Addr]])] = if (abs.isFalse(fcond)) {
                  MayFailError(List(TypeError(name, "first operand", "list", "non-list")))
                } else { listMfmon.zero }
                listMfmon.append(ft, ff)
              })
            } else { MayFailSuccess((List[Abs](), Set[Effect[Addr]]())) }
      ...
      if (args.length != 2) {
        ...
        val request: PrimitiveResultMF[Abs, Addr] = functionArgsList.map[PrimitiveResultType[Abs, Addr]]({
          case (functionArgs, effects) =>
            val fromPrims: Set[PrimitiveReturn[Abs, Addr]] = sabs.getPrimitives(arg1._2).map( (prim) => {
              StartFunctionCallRequest(arg1._1, Left(prim), functionArgs, store, DummyPrimitiveAppState)
            })
            val fromClosures: Set[PrimitiveReturn[Abs, Addr]] = sabs.getClosures(arg1._2).map( (clo) => {
              StartFunctionCallRequest(arg1._1, Right(clo), functionArgs, store, DummyPrimitiveAppState)
            })
            (fromPrims ++ fromClosures, store, effects)
        })
        request
      }
    }
  ...
  object Map extends Primitive[Addr, Abs] {
    ...
    implicit object PrimReachedMonoid
      extends Monoid[PrimitiveReachedType[Abs, Addr]] {
      def zero = (Set(), Store.empty[Addr, Abs], Set())
      def append(x: PrimitiveReachedType[Abs, Addr],
                 y: => PrimitiveReachedType[Abs, Addr]): PrimitiveReachedType[Abs, Addr] = {
        (x._1 ++ y._1, x._2.join(y._2), x._3 ++ y._3)
      }
    }
    ...
    val mfmonReached = MayFail.monoid[PrimitiveReachedType[Abs, Addr]]

    val name = "map"

    sealed trait MapState[Exp, Abs, Addr] extends PrimitiveAppState {
      def fexp: Exp
    ...
    case class MapStatePrim[Exp : Expression](fexp: Exp,
                                              accumulatedList: Abs,
                                              remainingList: Abs,
                                              primitive: Primitive[Addr, Abs])
      extends MapState[Exp, Abs, Addr]
    case class MapStateClo[Exp: Expression](fexp: Exp,
                                            accumulatedList: Abs,
                                            remainingList: Abs,
            ...
            car(arg2._2, store).bind({
              case (carl, effects1) =>
                cdr(arg2._2, store).bind({
                  case (cdrl, effects2) =>
                    val fromPrims: Set[PrimitiveReturn[Abs, Addr]] = sabs.getPrimitives[Addr, Abs](arg1._2).map( (prim) => {
                      StartFunctionCallRequest(arg1._1, Left(prim), List(carl), store, MapStatePrim(fexp, sabs.nil,
                        cdrl, prim))
                    })
                    val fromClosures: Set[PrimitiveReturn[Abs, Addr]] = sabs.getClosures[Exp, Addr](arg1._2).map( (clo) => {
                      StartFunctionCallRequest(arg1._1, Right(clo), List(carl), store, MapStateClo(fexp, sabs.nil,
                        cdrl, clo))
                    })
                    MayFailSuccess[PrimitiveResultType[Abs, Addr]]((fromPrims ++ fromClosures, store, effects1 ++ effects2))
                })
            })
          } else { mfmon.zero }
          mfmon.append(t, f)

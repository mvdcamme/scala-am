object BenchmarksConfig {
  ...
  val parser = new scopt.OptionParser[Configuration]("scala-am") {
    ...
    opt[Int]('w', "workers") action { (x, c) =>
      c.copy(workers = x)
    } text ("Number of workers to run the benchmarks on (1 by default)")
    opt[Config.Time]('t', "timeout") action { (x, c) =>
...
abstract class Benchmarks(dir: String,
    ...
    def compute(config: MachineConfig): MachineOutput = {
      ...
      if (program != null || program.size > 0) {
        try {
          ...
          case e: Throwable => {
            println(
              s"Benchmark ${config.program} failed! (config: $config, error: $e)")
  ...
  class Dispatcher(bound: Option[Int]) extends Actor {
    ...
    private case class State(computing: Int,
                             work: Queue[MachineConfig],
                             results: Results)

    private def sendWork(actor: ActorRef, state: State): Receive =
      state.work.dequeueOption match {
        case Some((item, remainingWork)) =>
          actor ! Computation(item)
          val newState =
            state.copy(computing = state.computing + 1, work = remainingWork)
          active(newState)
        case None => {
  ...
  def main(args: Array[String]) {
    BenchmarksConfig.parser
      .parse(args, BenchmarksConfig.Configuration()) match {
...
object MonoBenchmarks
    extends Benchmarks(
      "test", {
        val programs = List("ack",
                            "blur",
                            "church",
                            "collatz",
                            "count",

...
case class AmbProgramState[Exp: Expression](normalState: ProgramState[Exp],
  ...
  def wrapApplyAction(sem: SemanticsTraced[Exp,
    ...
    : ActionReturn[Exp,
                   ...
                   HybridTimestamp.T,
                   AmbProgramState[Exp]] =
    normalState.applyAction(sem, action) match {
      ...
      case GuardFailed(rp, guardID) =>
        GuardFailed[Exp,
                    ...
                    HybridAddress.A,
                    HybridTimestamp.T,
                    AmbProgramState[Exp]](rp, guardID)
      case TraceEnded(rp) =>
        TraceEnded[Exp,
                   ConcreteValue,
                   HybridAddress.A,
                   HybridTimestamp.T,
                   AmbProgramState[Exp]](rp)
    }
  ...
  def addFailActions(
    ...
    : ActionReturn[Exp,
                   ...
                   HybridTimestamp.T,
                   AmbProgramState[Exp]] = {
    val step: ActionReturn[Exp,
                           ConcreteValue,
                           HybridAddress.A,
                           HybridTimestamp.T,
                           ProgramState[Exp]] =
      normalState.applyAction(sem, action)
    step match {
      case ActionStep(state, action) =>
        val failureFrames = failActions.map({ action =>
          UndoActionFrame(action)
        })
  ...
  def convertState[AbstL: IsConvertableLattice](
    ...
    normalState.convertState[AbstL](concSem, abstSem, initialKontAddress, mapKontAddress)
  ...
  def runHeader(sem: SemanticsTraced[Exp,
    ...
    normalState
      ...
      .fold(None: Option[AmbProgramState[Exp]])(programState =>
        Some(AmbProgramState(programState, failStack)))
  ...
  def restart(sem: SemanticsTraced[Exp,
    ...
    : AmbProgramState[Exp] = restartPoint match {
  ...
  def step(
    ...
    someStep match {
      ...
      case Some(step) =>
        ...
        type PopKontAction =
          ActionSinglePopKontT[Exp, ConcreteValue, HybridAddress.A]
        ...
        someActionSinglePopKont match {
          ...
          case Some(actionSinglePopKont) =>
            val someTopKont =
              normalState.kstore.lookup(normalState.a).headOption
            someTopKont match {
              case None => Some(step)
              case Some(topKont) =>
                ...
                someBody match {
                  case None => Some(step)
                  case Some(body) =>
                    val signal = SignalEndLoop(
                      body,
                      RestartStoppedInBacktrack[Exp,
                                                ConcreteValue,
                                                HybridAddress.A]())
  ...
  def applyAction(sem: SemanticsTraced[Exp,
    ...
    : ActionReturn[Exp,
                   ...
                   HybridAddress.A,
                   HybridTimestamp.T,
                   AmbProgramState[Exp]] = action match {
    ...
    case ActionExtendEnvT(_) =>
      addFailAction(sem,
                    action,
                    ActionSingleSaveValT[Exp, ConcreteValue, HybridAddress.A](
                      normalState.vStack.head.getVal))
    case ActionPopFailKontT() =>
      failStack match {
        case head :: tail =>
          ...
          val extendedKStore =
            normalState.kstore.extend(next, Kont(head, normalState.a))
          val newNormalState = normalState.copy(control =
                                                  TracingControlKont(next),
    ...
    case ActionPopKontT() =>
      if (normalState.a == HaltKontAddress) {
        ...
        val failAction =
          ActionSinglePushKontT[Exp, ConcreteValue, HybridAddress.A](
    ...
    case ActionPushFailKontT(failureFrame) =>
      ActionStep(AmbProgramState(normalState, failureFrame :: failStack),
                 action)
    case ActionPushValT() =>
      addFailAction(
        sem,
        action,
        ActionSingleRestoreValT[Exp, ConcreteValue, HybridAddress.A]())
    case ActionRestoreEnvT() =>
      addFailAction(
        sem,
        action,
        ActionSingleSaveSpecificEnvT[Exp, ConcreteValue, HybridAddress.A](
          normalState.vStack.head.getEnv,
          normalState.ρ))
    case ActionSaveEnvT() =>
      addFailAction(
        sem,
    ...
    case ActionStepInT(fexp, e, args, argsv, n, frame, _, _) =>
      ...
      val failActions = ActionSinglePopKontT[Exp,
                                             ConcreteValue,
                                             HybridAddress.A]() ::
          ActionSingleRestoreEnvT[Exp, ConcreteValue, HybridAddress.A]() ::
            actionsSaveVal
    ...
    case ActionSinglePushKontT(frame) =>
      ...
      val extendedKStore =
        normalState.kstore.extend(next, Kont(frame, normalState.a))
      val newNormalState = normalState
        .copy(kstore = extendedKStore, a = next, t = time.tick(normalState.t))
    ...
    case ActionSingleRestoreValT() =>
      normalState.vStack match {
        case head :: rest =>
    ...
    case ActionSingleSaveSpecificEnvT(ρToSave, ρToReplace) =>
      val newNormalState =
        normalState.copy(ρ = ρToReplace,
                         vStack = StoreEnv[ConcreteValue, HybridAddress.A](
                             ρToSave) :: normalState.vStack)
      ActionStep(AmbProgramState(newNormalState, failStack), action)
    case ActionSingleSaveValT(value) =>
      val newNormalState =
        normalState.copy(vStack = StoreVal[ConcreteValue, HybridAddress.A](
            value) :: normalState.vStack)
      ActionStep(AmbProgramState(newNormalState, failStack), action)

class Storable[Abs: JoinLattice, Addr: Address](combo: Either[Abs, Environment[Addr]]) {
  def getVal: Abs = combo match {
    ...
    case Right(env) => throw new IncorrectStorableException(s"Environment $env is not a value")
  }
  def getEnv: Environment[Addr] = combo match {
    case Left(value) => throw new IncorrectStorableException(s"Value $value is not an environment")
    case Right(env) => env
  }
}

...
/*
 * TODO This is the old implementation of the AAM, to be used if an AAM without an IsSchemeLattice typeclass for
 * Abs is required. Refactor this.
 */
class AAMJoinLattice[Exp : Expression, Abs : JoinLattice, Addr : Address, Time : Timestamp]
  ...
  case class State(control: Control, store: Store[Addr, Abs], kstore: KontStore[KontAddr], a: KontAddr, t: Time) {
    ...
    def step(sem: Semantics[Exp, Abs, Addr, Time]): Set[State] = control match {
      ...
      case ControlEval(e, env) => integrate(a, sem.stepEval(e, env, store, t).map(_.action))
      /* In a continuation state, call the semantics' continuation method */
      case ControlKont(v) => kstore.lookup(a).flatMap({
        case Kont(frame, next) => integrate(next, sem.stepKont(v, frame, store, t).map(_.action))
      })

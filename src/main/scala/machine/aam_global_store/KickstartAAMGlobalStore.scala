...
case class KickstartAAMState[Exp: Expression, Abs: IsSchemeLattice, Addr: Address, Time : Timestamp](control: Control[Exp, Abs, Addr], a: KontAddr, t: Time) extends StateTrait[Exp, Abs, Addr, Time] {

  type G = aam_global_store.G[Exp, Abs, Addr, Time]
  type Tup = aam_global_store.Tup[Exp, Abs, Addr, Time]

  override def toString = control.toString
  def subsumes(that: KickstartAAMState[Exp, Abs, Addr, Time]): Boolean = control.subsumes(that.control) && a == that.a && t == that.t
  ...
  private def integrate(a: KontAddr, edgeInfos: Set[EdgeInformation[Exp, Abs, Addr]], store: GlobalStore[Addr, Abs], kstore: KontStore[KontAddr]): (Set[Tup], GlobalStore[Addr, Abs], KontStore[KontAddr]) =
    edgeInfos.foldLeft((Set[Tup](), store, kstore))((acc, edgeInfo) => {
      ...
      implicit def stateToTup(state: KickstartAAMState[Exp, Abs, Addr, Time]): Tup = (edgeAnnot, state)

      edgeInfo.action match {
        case ActionReachedValue(v, store2, _) => (acc._1 + KickstartAAMState(ControlKont(v), a, Timestamp[Time].tick(t)), acc._2.includeDelta(store2.delta), acc._3)
        case ActionPush(frame, e, env, store2, _) =>
          val next = NormalKontAddress[Exp, Time](e, t)
          (acc._1 + KickstartAAMState(ControlEval(e, env), next, Timestamp[Time].tick(t)), acc._2.includeDelta(store2.delta), acc._3.extend(next, Kont(frame, a)))
        case ActionEval(e, env, store2, _) =>
          (acc._1 + KickstartAAMState(ControlEval(e, env), a, Timestamp[Time].tick(t)), acc._2.includeDelta(store2.delta), acc._3)
        case ActionStepIn(fexp, _, e, env, store2, _, _) => //TODO Shouldn't Timestamp.tick include fexp here???
          (acc._1 + KickstartAAMState(ControlEval(e, env), a, Timestamp[Time].tick(t, fexp)), acc._2.includeDelta(store2.delta), acc._3)
        case ActionError(err) =>
          (acc._1 + KickstartAAMState(ControlError(err), a, Timestamp[Time].tick(t)), acc._2, acc._3)
      }
    })
...
object KickstartAAMState {
  def inject[Exp: Expression, Abs: IsSchemeLattice, Addr: Address, Time: Timestamp](exp: Exp, env: Iterable[(String, Addr)], store: Iterable[(Addr, Abs)]): FullState[Exp, Abs, Addr, Time] =
    (KickstartAAMState(ControlEval[Exp, Abs, Addr](exp, Environment.initial[Addr](env)), HaltKontAddress, Timestamp[Time].initial("")),
  ...
  implicit def graphNode[Exp: Expression, Abs: IsSchemeLattice, Addr: Address, Time: Timestamp] = new GraphNode[KickstartAAMState[Exp, Abs, Addr, Time], Context[Exp, Abs, Addr, Time]] {
    override def label(s: KickstartAAMState[Exp, Abs, Addr, Time]) = s.toString.take(40)
...
class KickstartAAMGlobalStore[Exp: Expression, Abs: IsSchemeLattice, Addr: Address, Time: Timestamp]
  ...
  case class AAMOutput(halted: Set[KickstartAAMState[Exp, Abs, Addr, Time]], finalStore: Store[Addr, Abs], numberOfStates: Int, time: Double,
                       errorStates: Set[KickstartAAMState[Exp, Abs, Addr, Time]], graph: G[Exp, Abs, Addr, Time], timedOut: Boolean, stepSwitched: Option[Int])
  ...
  def kickstartEval(initialState: InitialState, sem: ConvertableSemantics[Exp, Abs, Addr, Time], stopEval: Option[KickstartAAMState[Exp, Abs, Addr, Time] => Boolean],
                    timeout: Timeout, stepSwitched: Option[Int]): AAMOutput = {
    import scala.language.higherKinds
    @scala.annotation.tailrec
    def loop[VS[_]: VisitedSet](todo: Set[KickstartAAMState[Exp, Abs, Addr, Time]], visited: VS[KickstartAAMState[Exp, Abs, Addr, Time]], store: GlobalStore[Addr, Abs], kstore: KontStore[KontAddr],
      halted: Set[KickstartAAMState[Exp, Abs, Addr, Time]], graph: G[Exp, Abs, Addr, Time], reallyVisited: Set[KickstartAAMState[Exp, Abs, Addr, Time]]): AAMOutput = {
      if (todo.isEmpty || timeout.reached) {
        ...
        val (edges, store2, kstore2) = todo.foldLeft(Set[(KickstartAAMState[Exp, Abs, Addr, Time], EdgeAnnotation[Exp, Abs, Addr], KickstartAAMState[Exp, Abs, Addr, Time])](), store, kstore)((acc, state) =>
        ...
        if (store2.isUnchanged && kstore.fastEq(kstore2)) {
          ...
          loop(edges.map({ case (s1, e, s2) => s2 }),
            VisitedSet[VS].empty[KickstartAAMState[Exp, Abs, Addr, Time]],
    ...
    loop(Set(state),
      ...
      store, kstore, Set(),
      Graph.empty[KickstartAAMState[Exp, Abs, Addr, Time], EdgeAnnotation[Exp, Abs, Addr], KickstartAAMState.Context[Exp, Abs, Addr, Time]].addNode(state), Set())
  }
  ...
  def eval(exp: Exp, sem: ConvertableSemantics[Exp, Abs, Addr, Time], graph: Boolean, timeout: Timeout): Output = {
    val initialState: InitialState = KickstartAAMState.inject[Exp, Abs, Addr, Time](exp, sem.initialEnv, sem.initialStore)

...
object ContextSensitiveTID {
  ...
  case class TID[Exp: Expression, Time: Timestamp](exp: Exp, t: Time)
      extends ContextSensitiveTID {
    override def toString =
      if (implicitly[Timestamp[Time]].name == "Concrete") {
        t.toString
      } else {
        exp.toString
      }
  ...
  implicit object CSTIDThreadIdentifier
      extends ThreadIdentifier[ContextSensitiveTID] {

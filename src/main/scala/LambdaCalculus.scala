...
object LamLatticeImpl {
  ...
  case class Closure[Exp: Expression, Addr: Address](exp: Exp,
  ...
  implicit val isLamLattice: LamLattice[L] = new LamLattice[L] {
    ...
    def inject[Exp: Expression, Addr: Address](
        x: (Exp, Environment[Addr])): L =
      L(Set[Value](Closure[Exp, Addr](x._1, x._2)))
    ...
    def getClosures[Exp: Expression, Addr: Address](x: L) = {
      def getClo(x: Value): (Exp, Environment[Addr]) = x match {
        case Closure(exp: Exp @unchecked, env: Environment[Addr] @unchecked) =>
          (exp, env)
      }
...
class LamSemantics[Abs: LamLattice, Addr: Address, Time: Timestamp]
  ...
  def stepKont(v: Abs, frame: Frame, store: Sto, t: Time) = frame match {
    ...
    case FrameFun(fun) =>
      labs
        .getClosures[LamExp, Addr](fun)
        .map({
          case (Lam(x, e, _), env) => {
            val a = addr.variable(x, v, t)
            noEdgeInfos(ActionEval[LamExp, Abs, Addr](e, env.extend(x, a), store.extend(a, v)), Nil)
          }
        })
  }
  ...
  def parse(program: String): LamExp = {
    def compile(exp: SExp): LamExp = exp match {
      /* case: (lambda (x) e) */
      case SExpPair(
          SExpIdentifier("lambda", _),
          SExpPair(SExpPair(SExpIdentifier(arg, _), SExpValue(ValueNil, _), _),
                   SExpPair(body, SExpValue(ValueNil, _), _),
                   _),
          _) =>
      ...
      case SExpPair(operator,
                    SExpPair(operand, SExpValue(ValueNil, _), _),
                    _) =>
...
case class UnboundVariablesAnalysis[
    Abs: JoinLattice, Addr: Address, Time: Timestamp]()
    extends BaseAnalysis[Set[LamExp], LamExp, Abs, Addr, Time] {

  /** stepEval is called when the semantics' stepEval is called */
  def stepEval(e: LamExp,
               env: Environment[Addr],
               store: Store[Addr, Abs],
               t: Time,
               current: Set[LamExp]) = e match {
...
object LamAnalysis {
  ...
  def analyze[L: LamLattice](program: String): Set[LamExp] = {
    ...
    machine.analyze(sem.parse(program),
                    sem,
                    analysis,
  ...
  def main(args: Array[String]) {
    ...
    val unbound =
      analyze[LamLatticeImpl.L]("((lambda (x) (x x)) (lambda (y) z))")
    /* We can extract positional information from the expressions of unbound variables */
    val unboundstr =
      unbound.map(x => s"$x at position ${x.pos}").mkString("\n")

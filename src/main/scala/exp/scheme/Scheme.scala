...
object SchemeDesugarer {
  ...
  def desugarExp(exp: SchemeExp): SchemeExp = {
    exp match {
      ...
      case SchemeLet(bindings, body, pos) => SchemeLet(bindings.map({ case (b, v) => (b, desugarExp(v)) }), body.map(desugarExp), pos)
      case SchemeLetStar(bindings, body, pos) => SchemeLetStar(bindings.map({ case (b, v) => (b, desugarExp(v)) }), body.map(desugarExp), pos)
      case SchemeLetrec(bindings, body, pos) => SchemeLetrec(bindings.map({ case (b, v) => (b, desugarExp(v)) }), body.map(desugarExp), pos)
      case SchemeSet(variable, value, pos) => SchemeSet(variable, desugarExp(value), pos)
      case SchemeBegin(exps, pos) => SchemeBegin(exps.map(desugarExp), pos)
      case SchemeCond(clauses, pos) => SchemeCond(clauses.map({ case (cond, body) => (undefine1(cond), undefineBody(body)) }), pos)
      case SchemeCase(key, clauses, default, pos) => SchemeCase(desugarExp(key), clauses.map({ case (vs, body) => (vs, body.map(desugarExp)) }), default.map(desugarExp), pos)
      case SchemeAnd(args, pos) => SchemeAnd(args.map(undefine1), pos)
      case SchemeOr(args, pos) => SchemeOr(args.map(undefine1), pos)
      case SchemeIdentifier(name, pos) => SchemeIdentifier(name, pos)
      case SchemeQuoted(quoted, pos) => SchemeQuoted(quoted, pos)
      case SchemeValue(value, pos) => SchemeValue(value, pos)
      case SchemeCas(variable, eold, enew, pos) => SchemeCas(variable, eold, enew, pos)
      case SchemeCasVector(variable, index, eold, enew, pos) => SchemeCasVector(variable, desugarExp(index), desugarExp(eold), desugarExp(enew), pos)

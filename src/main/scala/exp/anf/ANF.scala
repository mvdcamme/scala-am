...
case class ANFLambda(args: List[String], body: ANFExp, pos: Position)
    extends ANFAtomicExp {
  override def toString() = {
    val a = args.mkString(" ")
    s"(lambda ($a) $body)"
  }
}
case class ANFIf(cond: ANFAtomicExp, cons: ANFExp, alt: ANFExp, pos: Position)
    extends ANFExp {
  override def toString() = s"(if $cond $cons $alt)"
}
case class ANFFuncall(f: ANFAtomicExp, args: List[ANFAtomicExp], pos: Position)
    extends ANFExp {
  override def toString() = {
    val a = args.mkString(" ")
    s"($f $a)"
...
case class ANFLet(variable: String, value: ANFExp, body: ANFExp, pos: Position)
    extends ANFExp {
  override def toString() = s"(let (($variable $value)) $body)"
}
case class ANFLetrec(variable: String,
                     value: ANFExp,
                     body: ANFExp,
                     pos: Position)
    extends ANFExp {
...
case class ANFSet(variable: String, value: ANFAtomicExp, pos: Position)
    extends ANFExp {
...
object ANFCompiler {
  ...
  def compile(exp: SchemeExp,
              tail: Boolean,
              k: ANFAtomicExp => ANFExp): ANFExp = exp match {
    ...
    case SchemeFuncall(f, args, pos) =>
      compile(
        f,
        false,
        a => compileList(args, as => ret(ANFFuncall(a, as, pos), tail, k)))
    case SchemeIf(cond, cons, alt, pos) =>
      compile(cond,
              false,
              a => ret(ANFIf(a, compile(cons), compile(alt), pos), tail, k))
    ...
    case SchemeDefineFunction(name, args, body, pos) =>
      compile(SchemeDefineVariable(name, SchemeLambda(args, body, pos), pos),
              tail,
              k)
    ...
    case SchemeAnd(e :: es, pos) =>
      compile(e,
              ...
              a =>
                ret(ANFIf(a,
                          compile(SchemeAnd(es, pos)),
                          ANFValue(ValueBoolean(false), pos),
                          pos),
                    tail,
                    k))
    ...
    case SchemeOr(e :: es, pos) =>
      compile(e,
              ...
              a =>
                ret(ANFIf(a,
                          ANFValue(ValueBoolean(true), pos),
                          compile(SchemeOr(es, pos)),
                          pos),
                    tail,
                    k))
  ...
  def compileBody(body: List[SchemeExp],
                  pos: Position,
                  k: ANFAtomicExp => ANFExp): ANFExp = body match {
  ...
  def compileList(exps: List[SchemeExp],
                  k: List[ANFAtomicExp] => ANFExp): ANFExp =
    compileListHelper(exps, k, List())
  def compileListHelper(exps: List[SchemeExp],
                        k: List[ANFAtomicExp] => ANFExp,
                        acc: List[ANFAtomicExp]): ANFExp = exps match {
    case Nil => k(acc.reverse)
    case exp :: rest =>
      compile(exp, false, e => compileListHelper(rest, k, e :: acc))
  }
}

...
abstract class StorePropSpec[Addr: Address](val lattice: SchemeLattice)
  ...
  val absint: Gen[Abs] = for { n: Int <- Gen.choose(-100, 100) } yield
    abs.inject(n)
  ...
  val abstractValues: Gen[Abs] =
    Gen.oneOf(absint, absstr, absbool, abschar, abssym)

  property("lookup after extend yields same object") {
    forAll(abstractValues) { v =>
  ...
  property(
    "store with abstract counting extended twice or more remains the same") {
    forAll(abstractValues) { v =>
      val a = addr.primitive("foo")
...
class ConcreteCountingStoreTest
    extends StorePropSpec[ClassicalAddress.A](new ConcreteLattice(true))
class ConcreteNoCountingStoreTest
    extends StorePropSpec[ClassicalAddress.A](new ConcreteLattice(false))
class TypeSetStoreTest
    extends StorePropSpec[ClassicalAddress.A](new TypeSetLattice(false))
